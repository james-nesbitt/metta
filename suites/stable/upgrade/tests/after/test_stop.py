...
from mirantis.testing.metta_mirantis.msr_client import MSRAPIClientPlugin, MSRReplicaHealth
from mirantis.testing.metta_mirantis.mke_client import MKEAPIClientPlugin, MKENodeState

from mirantis.testing.metta_kubernetes.deployment_workload import KubernetesDeploymentWorkloadInstance
...
def test_msr_replica_health(msr_client: MSRAPIClientPlugin):
    ...
    for replica_id, replica_health in status['replica_health'].items():
        assert MSRReplicaHealth.OK.match(
            replica_health), f"Replica [{replica_id}] is not READY"
