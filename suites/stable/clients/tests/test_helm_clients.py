...
def test_kubernetes_helm_workload(environment_up):
    ...
    try:
        ...
        nsd = apps_v1.list_namespaced_deployment(
            namespace=DEFAULT_K8S_NAMESPACE)
        logger.info("Namespace (%s) deployments: %s", DEFAULT_K8S_NAMESPACE,
        ...
        deployment = apps_v1.read_namespaced_deployment(f"{instance.name}-metrics-server",
                                                        instance.namespace)
