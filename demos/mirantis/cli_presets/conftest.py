...
from mirantis.testing.mtt_mirantis.presets import MTT_MIRANTIS_PRESETS, MTT_MIRANTIS_PRESET_CONFIG_LABEL, add_preset_config
...
def pytest_addoption(parser):
    """ Add pytest cli options to allow all mtt_mirantis_presets """
    ...
    for preset in MTT_MIRANTIS_PRESETS:
        ...
        parser.addoption(PRESET_FLAG_TEMPLATE.format(preset_key), action="store", default=preset_default)
...
def config(request):
    ...
    Bootstrap for:
    - mtt_mirantis: add some common config
    ...
    mtt_mirantis operation for detecting presets
    ...
    # discover an mtt_mirantis presets that we set as pytest cli flags
    mtt_mirantis_preset_config = {}
    for preset in MTT_MIRANTIS_PRESETS:
        preset_key = preset[0]
        preset_value = request.session.config.getoption(PRESET_FLAG_TEMPLATE.format(preset_key))
        if preset_value:
            mtt_mirantis_preset_config[preset_key] = preset_value
    # Add a config source that sets the prests in the manner that mtt_mirantis expects
    # { mtt_mirantis: { [preset_key]: [preset_value]... }}
    config.add_source(mtt.CONFIGSOURCE_DICT, 'cli_presets', priority=config.default_priority()+1).set_data(
        MTT_MIRANTIS_PRESET_CONFIG_LABEL: mtt_mirantis_preset_config
    )

    # tell mtt_mirantis to look for presets again, to discover our new presets
    config.bootstrap('mtt_mirantis_presets')
