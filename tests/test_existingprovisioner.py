...
from mirantis.testing.mtt_common.plugins.provisioner import ExistingBackendProvisionerPlugin
...
def existing_config():
    ...
    return {
        'provisioner': {
            'plugin_id': mtt_common.MTT_PLUGIN_ID_PROVISIONER_EXISTING,
            'outputs': {
                'main': {
                    "one": 1
                }
...
def config(existing_config):
    ...
    conf = mtt.new_config()
    conf.add_source(mtt_common.MTT_PLUGIN_ID_CONFIGSOURCE_DICT).set_data(existing_config)
...
def test_existing_clients(existing_config, provisioner):
    ...
    client_main = provisioner.client('main')
