    ...
    def __call__(self, func):
        ...
        def matrix_wrap(config: Config, provisioner):
            ...
            for combination in self.matrix:
                ...
                mtt_dict = {}
                if combination[0]:
                    mtt_dict["platform"] = combination[0]
                if combination[1]:
                    mtt_dict["cluster"] = combination[1]
                if combination[2]:
                    mtt_dict["release"] = combination[2]

                id = str("-".join(mtt_dict.values())).replace("/", "-")
                ...
                combination_config.add_source(mtt.SOURCE_DICT, "combination", config.default_priority() + 1).set_data({
                    "mtt": mtt_dict,
                    "variables": {
                ...
                # tell mtt to add variation/platform/release/cluster
                add_preset_config(combination_config)
                # build a new mtt provisioner now that we have added config
                combination_provisioner = mtt.new_provisioner_from_config(
