...
from mirantis.testing.mtt import CONFIGSOURCE_DICT, CONFIGSOURCE_PATH

logger = logging.getLogger('mtt_mirantis.presets')

MTT_MIRANTIS_PRESET_CONFIG_LABEL = 'mtt_mirantis'
...
MTT_MIRANTIS_CONFIG_SOURCE_INSTANCE_ID_PREFIX = 'mtt_mirantis_preset'
""" All config that we add here will have its source instance_id prefixed with this """
MTT_MIRANTIS_PRESETS = [
    ('variation', pkg_resources.resource_filename('mirantis.testing.mtt_mirantis', 'config/variation')),
    ('cluster', pkg_resources.resource_filename('mirantis.testing.mtt_mirantis', 'config/cluster')),
    ('platform', pkg_resources.resource_filename('mirantis.testing.mtt_mirantis', 'config/platform')),
    ('release', pkg_resources.resource_filename('mirantis.testing.mtt_mirantis', 'config/release'))
]
""" List of available presets that you can include in load('mtt_mirantis')
...
"""
MTT_MIRANTIS_PRESET_DEFAULT_PRIORITY = 70
""" Default priority to use for preset config sources """

def add_preset_config(config:Config, priority=MTT_MIRANTIS_PRESET_DEFAULT_PRIORITY):
    """ Read mtt_mirantis config and interpret it for modifying the config

    Interprests config.load('mtt_mirantis') to determine if any `presets` config
    ...
    In all cases, the pattern is that a key from config.load('mtt_mirantis') is
    ...
    # To do this we load the `mtt_mirantis` config label and look for preset
    # keys in the loaded config.
    # For each preset key found we try to add a path config source for the value
    mtt_mirantis_config = config.load(MTT_MIRANTIS_PRESET_CONFIG_LABEL)

    for preset in MTT_MIRANTIS_PRESETS:
        # For each of the available presets, look in the loaded mtt_mirantis
        # config for values matching the preset key.  if the key is found then
        # we try to match a path in the mtt_mirantis/config/{key}/{value}
        # and add the path as a config source.

        (preset_key, preset_root_path) = preset
        preset_value = mtt_mirantis_config.get(preset_key, exception_if_missing=False)
        if preset_value:
            preset_instance_id = '{}-{}-{}'.format(MTT_MIRANTIS_CONFIG_SOURCE_INSTANCE_ID_PREFIX, preset_key, preset_value)
            # quick check to see if we've already added this preset.
            if not config.has_source(preset_instance_id):
                # build a preset config path and add it as a source if it exists
                preset_full_path = os.path.join(preset_root_path, preset_value)
                if os.path.isdir(preset_full_path):
                    config.add_source(CONFIGSOURCE_PATH, preset_instance_id, priority).set_path(preset_full_path)
                else:
                    raise KeyError("mtt_mirantis doesn't have a preset '%s:%s'", preset_key, preset_value)
