...
logger = logging.getLogger("MTT Mirantis Matrix run")
...
class MatrixRun:
    ...
    def __call__(self, func):
        ...
        def matrix_wrap(config:Config, provisioner):
            ...
            for combination in self.matrix:
                ...
                mtt_mirantis_dict = {}
                if combination[0]:
                    mtt_mirantis_dict['platform'] = combination[0]
                if combination[1]:
                    mtt_mirantis_dict['cluster'] = combination[1]
                if combination[2]:
                    mtt_mirantis_dict['release'] = combination[2]

                id = str("-".join(mtt_mirantis_dict.values())).replace('/', '-')
                logger.debug("MatrixTestRun::Combination: %s", id)

                mtt_mirantis_dict['id'] = "{}-{}".format(variables_config.get('id'), id).replace('/', '-')

                combination_config = config.copy()
                combination_config.add_source(mtt.CONFIGSOURCE_DICT, 'combination', config.default_priority()+1).set_data({
                    'mtt_mirantis': mtt_mirantis_dict,
                    'variables' : {
                ...
                # tell mtt_mirantis to add variation/platform/release/cluster
                add_preset_config(combination_config)
                # build a new mtt provisioner now that we have added config
                combination_provisioner = mtt.new_provisioner_from_config(combination_config, 'matrix_provisioner')

                thread = ProvisionerUpFuncCallThread(id, combination_config, combination_provisioner, func)
                thread.start()
...
class ProvisionerUpFuncCallThread(Thread):
    ...
    def __init__(self,id, config, provisioner, func):
        self.logger = logger.getChild("{}:matrix:{}".format(func.__code__.co_name, id))
        self.id = id
        self.config = config
    ...
    def run(self):
        ...
        try:
            self.logger.info("Preparing the testing cluster using the provisioner")
            self.provisioner.prepare()
        except Exception as e:
            raise Exception("Provisioner failed to prepare()") from e
        try:
            self.logger.info("Starting up the testing cluster using the provisioner")
    ...
    def run2(self):
        ...
        can handle it.
        """
        self.logger.info("Running MTT provisioner up()", id)
        ...
        if conf.get("options.destroy-on-finish", exception_if_missing=False):
            try:
                self.logger.info("Stopping the test cluster using the provisioner as directed by config")
                self.provisioner.destroy()
            except Exception as e:
                raise Exception("Provisioner failed to destroy()") from  e
        else:
            self.logger.info("Leaving test infrastructure in place on shutdown")
